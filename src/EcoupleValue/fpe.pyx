    ...
    for i in range(N):
        for j in range(N):
            drift_at_pos[0, i, j] = drift1(
                positions[i], positions[j],
                n1, n2, phase, E0, Ecouple, mu_Hp,
                m1, gamma1
                )
            drift_at_pos[1, i, j] = drift2(
                positions[i], positions[j],
                n1, n2, E1, Ecouple, mu_atp,
                m2, gamma2
                )
...
cdef double potential(
    ...
    return 0.5*(
        ...
        + Ecouple*(1-cos(n1*position1-n2*position2))
        + E1*(1-cos((n2*position2)))
        )

cdef double drift1(
    double position1, double position2, double n1, double n2,
    ...
    return (-1.0/(m1*gamma1))*((0.5)*(
        Ecouple*n1*sin(n1*position1-n2*position2)
...
cdef double drift2(
    ...
    return (-1.0/(m2*gamma2))*((0.5)*(
        (-1.0)*Ecouple*n2*sin(n1*position1-n2*position2)
...
cdef void update_probability_full(
    ...
    for i in prange(1, N-1, num_threads=6):
        # Periodic boundary conditions:
